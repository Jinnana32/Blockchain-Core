...
app.get("/info", (req, res) => {
  ...
  let info = {
    ...
    confirmedTransactions: Node.TransactionManager.getBlockTransactions()
      .length,
...
app.get("/debug/mineblock/:address", (req, res) => {
  ...
  let block = new Block(
    ...
    MagicLedger.currentDifficulty,
    nonce,
    MLTransactionManager.getPendingTransactions()
  );
...
app.get("/transactions/:transHash", (req, res) => {
  res.send(
    JSON.stringify(
      MLTransactionManager.getTransactionsByHash(req.params.transHash)
    )
  );
});
...
app.post("/transactions/send", (req, res) => {
  ...
  for (let x = 0; x < Node.nodes.length; x++) {
    ...
    axios
      .post(`${networkURL}/transactions/send/broadcast`, {
...
app.get("/transaction/address/:address", (req, res) => {
  res.send(
    JSON.stringify(
      MLTransactionManager.getTransactionByAddress(req.params.address)
    )
  );
});
...
app.get("/balance/address/:address", (req, res) => {
  res.send(
    JSON.stringify(MLTransactionManager.getBalanceByAddress(req.params.address))
  );
});
...
app.post("/mining/submit-block", (req, res) => {
  ...
  // if incorrect hash
  if (
    block.blockHash !== ProperData ||
    blockDataHash !== expectedBlockDataHash
  ) {
...
app.post("/debug/mine", (req, ress) => {
  ...
  axios
    .get(Node.networkUrl + "/mining/getjobs/" + minerAddress)
    .then(function(res) {
      ...
      let block = new Block(
        nextIndex,
        nextHash,
        previosBlock.previousHash,
        previosBlock.rewardAddress,
        nextTimeStamp,
        difficulty,
        nonce,
        previosBlock.transactions
      );
      ...
      axios
        .post(Node.networkUrl + "/mining/submit-block", nextBlock)
        .then(function(result) {
          ...
          for (let x = 0; x < Node.nodes.length; x++) {
            ...
            console.log(networkURL + " Transactions was successfully sync.");
            axios
              .post(`${networkURL}/block/broadcast`, { block: block })
              .then(function() {})
              .catch(function() {});
          }
        })
...
app.post("/connect-node", (req, res) => {
  ...
  if (Node.nodes.indexOf(connectionNodeUrl) == -1)
    Node.nodes.push(connectionNodeUrl);

  // Make a request to every node to register the new node
  for (let x = 0; x < Node.nodes.length; x++) {
    const networkURL = Node.nodes[x];
    axios
      .post(`${networkURL}/broadcast-node`, { nodeUrl: connectionNodeUrl })
      .then(function() {})
      .catch(function() {});
  }
  ...
  axios
    ...
    .then(function() {
      ...
      axios
        ...
        .then(function(result) {
          ...
          if (connectingDifficulty > currentDiffuculty) {
            axios.get(`${connectionNodeUrl}/blocks`).then(function(res) {
              Node.blockchain.updateChain(res.data);
              console.log(
                "[Synchronization]",
                "Discarded current chain. Peer chain is much longer."
              );
            ...
            axios
              .post(`${connectionNodeUrl}/sync-chain`, {
                blocks: Node.blockchain.chain
              })
              .then(function() {})
              .catch(function() {});
      ...
      console.log(
        "[Register]",
        `Node ${connectionNodeUrl} was added to the network.`
      );
    });
});
...
app.post("/broadcast-node", (req, res) => {
  const connectionNodeUrl = req.body.nodeUrl;
  if (
    !Node.nodeHash(connectionNodeUrl) &&
    !Node.isCurrentNode(connectionNodeUrl)
  )
    Node.nodes.push(connectionNodeUrl);
...
app.post("/register-nodes", (req, res) => {
  ...
  for (let x = 0; x < networks.length; x++) {
    ...
    if (!Node.nodeHash(nodeUrl) && !Node.isCurrentNode(nodeUrl))
      Node.nodes.push(nodeUrl);
  }
